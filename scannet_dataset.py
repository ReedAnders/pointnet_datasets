...
class ScannetDataset():
	...
	def __init__(self, root, num_classes, npoints=8192, split='train'):
		...
		self.subsample_dimensions_m = 100
...
class ScannetDatasetWholeScene_Pred():
	...
	def __getitem__(self, index):
		...
		points_default = self.scene_points_list[index]
		point_set_ini = self.scene_points_list[index]
		semantic_seg_ini = self.semantic_labels_list[index].astype(np.int32)
		coordmax = np.max(point_set_ini,axis=0)
		coordmin = np.min(point_set_ini,axis=0)
		...
		return point_sets, semantic_segs, sample_weights, points_default
